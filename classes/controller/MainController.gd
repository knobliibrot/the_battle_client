...
func _ready():
	self.act_scene = get_node("ActualScene")
	call_deferred("change_to_menu_scene")

func change_to_menu_scene() -> void:
	change_scene(MENU_SCENE)
	var _err = self.act_scene.connect("start_open_game", self, "on_MenuScene_start_open_game")
	var _err2 = self.act_scene.connect("start_open_mirrored_game", self, "on_MenuScene_start_open_mirrored_game")
	var _err3 = self.act_scene.connect("start_multiplayer_game", self, "on_MenuScene_start_multiplayer_game")

func change_to_open_game_scene() -> void:
	change_scene(OPEN_GAME_SCENE)
	var _err = self.act_scene.connect("ready_to_close", self, "on_GameScene_ready_to_close")
...
func change_scene(new_scene: PackedScene) -> void:
	...
	remove_child(self.act_scene)
	add_child(new_node)
	self.act_scene = new_node
