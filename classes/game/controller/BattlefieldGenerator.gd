...
# Generates the battlefield randomly but mirrored
func generate_mirrored_battlefield() -> Array:
	self.battlefield_map = []
	self.battlefield_map.resize(GameSettings.battlefield_height)
	for y in range(GameSettings.battlefield_height):
		var row: Array = []
		...
		for x in range(GameSettings.battlefield_width):
			...
			if (x + y) % 2 == 1:
				var field: Field
				# First column have just empty fields
				if x != 0 and x != GameSettings.battlefield_width -1:
					field = get_battlefield().initalize_field(Vector2(x,y), choose_field_type(x, y)) 
				else:
					field = get_battlefield().initalize_field(Vector2(x,y), FieldType.EMPTY) 
	...
	#Mirror the battelfield
	var new_x: int = GameSettings.battlefield_width - 1
	for x in range((GameSettings.battlefield_width - 1) / 2):
		for y in range(GameSettings.battlefield_height):
			if (x + y) % 2 == 1:
				var field_type: int = get_battlefield().get_field(Vector2(x, y)).field_type
				var field: Field = get_battlefield().initalize_field(Vector2(new_x,y), field_type) 
	...
	for y in range(GameSettings.battlefield_height):
		...
		for x in range(GameSettings.battlefield_width):
			...
			if (x + y) % 2 == 1:
				var field_type: int = get_battlefield().get_field(Vector2(x, y)).field_type
				var field: Field = get_battlefield().initalize_field(Vector2(x,y), field_type) 
